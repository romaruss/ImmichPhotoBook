...
- **Smart layout** — automatic page composition with orientation matching, event-cluster grouping, caption-aware slot selection
- **Duplicate removal** — perceptual hash (dHash) + burst-shot detection with AND logic; configurable similarity threshold
- **GPS map auto-fill** — empty slots filled with a static GPS cluster map (Stadia Maps / OSM fallback)
- **Pan/zoom/crop** — per-slot interactive adjustment with live preview; page-level zoom 30%–250%
...
- **Generation log** — detailed per-page/per-slot log with dedup comparisons, cluster separators, and map previews
